...
fn keylimectl_in_clean_dir(
    tmpdir: &tempfile::TempDir,
) -> Command {
    let mut cmd =
        Command::cargo_bin("keylimectl").unwrap(); //#[allow_ci]
...
fn test_generate_runtime_from_ima_log() {
    ...
    let mut f =
        std::fs::File::create(&ima_path).unwrap(); //#[allow_ci]
    writeln!(
        f,
    ...
    let output_path =
        tmpdir.path().join("runtime_policy.json");
    ...
    let content =
        std::fs::read_to_string(&output_path).unwrap(); //#[allow_ci]
...
fn test_generate_runtime_from_allowlist() {
    ...
    let allowlist_path =
        tmpdir.path().join("allowlist.txt");
    let mut f =
        std::fs::File::create(&allowlist_path).unwrap(); //#[allow_ci]
    writeln!(f, "abc123def456  /usr/bin/allowed1")
        .unwrap(); //#[allow_ci]
    writeln!(f, "789012345678  /usr/bin/allowed2")
        .unwrap(); //#[allow_ci]
    ...
    let content =
        std::fs::read_to_string(&output_path).unwrap(); //#[allow_ci]
    let policy: serde_json::Value =
        serde_json::from_str(&content).unwrap(); //#[allow_ci]
...
fn test_generate_runtime_with_excludelist() {
    ...
    let exclude_path =
        tmpdir.path().join("excludelist.txt");
    let mut f =
        std::fs::File::create(&exclude_path).unwrap(); //#[allow_ci]
    ...
    let excludes =
        policy["excludes"].as_array().unwrap(); //#[allow_ci]
    assert!(
        excludes.len() >= 2,
...
fn test_validate_valid_runtime_policy() {
    ...
    let policy_path =
        tmpdir.path().join("valid_policy.json");
...
fn test_validate_invalid_runtime_policy() {
    ...
    let policy_path =
        tmpdir.path().join("invalid_policy.json");
    let policy = serde_json::json!({
        "meta": {
            "version": 5,
            "generator": "keylimectl"
...
fn test_sign_and_verify_policy() {
    ...
    let policy = serde_json::json!({
        ...
        "release": 0,
        "digests": {"/usr/bin/test": ["sha256:aabbccdd"]},
        "excludes": []
    });
    std::fs::write(
        &policy_path,
    ...
    let signed_path =
        tmpdir.path().join("signed_policy.json");
    ...
    assert!(
        signed_path.exists(),
        "Expected signed policy file to exist"
    );
    assert!(
        key_path.exists(),
        "Expected generated key file to exist"
    );

    // Verify the signed policy file is valid JSON
    let signed_content =
        std::fs::read_to_string(&signed_path).unwrap(); //#[allow_ci]
    let signed: serde_json::Value =
        serde_json::from_str(&signed_content)
            .unwrap_or_else(|e| {
                panic!(
                    "Expected valid JSON signed envelope, got error: {e}"
                ) //#[allow_ci]
            });
...
fn test_sign_with_x509_backend() {
    ...
    let signed_path =
        tmpdir.path().join("signed_x509.json");
    ...
    assert!(
        signed_path.exists(),
        "Expected signed policy file"
    );
    assert!(
        cert_path.exists(),
        "Expected X.509 certificate file"
    );
}
...
fn test_convert_flat_allowlist() {
    ...
    assert!(
        output_path.exists(),
        "Expected converted policy file"
    );
...
fn test_convert_json_allowlist() {
    ...
    let allowlist_path =
        tmpdir.path().join("allowlist.json");
