...
fn keylimectl_in_clean_dir(tmpdir: &tempfile::TempDir) -> Command {
    ...
    // Isolate from system config files by restricting the search to
    // a single path inside the tmpdir (same pattern as KEYLIME_AGENT_CONFIG).
    cmd.env("KEYLIMECTL_CONFIG", tmpdir.path().join("keylimectl.toml"));
...
fn test_info_tls_with_missing_certs() {
    ...
    // Write a config that explicitly points to non-existent cert files
    // so the test does not depend on host filesystem state (the CI image
    // may have real certs at the default paths).
    let config_path = tmpdir.path().join("keylimectl.toml");
    std::fs::write(
        &config_path,
        r#"
[tls]
client_cert = "/nonexistent/client.crt"
client_key = "/nonexistent/client.pem"
trusted_ca = ["/nonexistent/ca.crt"]
"#,
    )
    .unwrap(); //#[allow_ci]
    ...
    assert!(
        !issues.is_empty(),
        "Expected issues when cert files don't exist"
    );
}
