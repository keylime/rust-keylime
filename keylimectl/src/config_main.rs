...
impl Config {
    ...
    fn get_config_paths(config_path: Option<&str>) -> Vec<PathBuf> {
        ...
        if let Some(home) = std::env::var_os("HOME") {
            ...
            paths.push(
                home_path.join(".config/keylimectl/config.toml"),
            );
        }
        ...
        if let Some(home) = std::env::var_os("HOME") {
            let home_path = PathBuf::from(&home);
            paths
                .push(home_path.join(".config/keylime/keylimectl.conf"));
            paths.push(home_path.join(".keylimectl.toml"));
        }
        if let Some(xdg_config) = std::env::var_os("XDG_CONFIG_HOME") {
            paths.push(
                PathBuf::from(xdg_config)
                    .join("keylime/keylimectl.conf"),
            );
        }
...
mod tests {
    ...
    fn create_test_cli(
        ...
        Cli {
            ...
            command: Some(crate::Commands::List {
