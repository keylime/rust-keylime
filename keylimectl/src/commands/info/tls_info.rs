...
pub fn execute(
    ...
    let cert_key_match = match (
        cfg.tls.client_cert.as_ref(),
        cfg.tls.client_key.as_ref(),
    ) {
    ...
    if !trusted_ca_info.is_empty() {
        certificates["trusted_ca"] =
            Value::Array(trusted_ca_info);
    }
...
fn inspect_certificate(
    ...
    let pem_data = match fs::read(p) {
        ...
        Err(e) => {
            issues.push(format!(
                "Cannot read certificate file {path}: {e}"
            ));
            return json!({
                "path": path,
                "exists": true,
    ...
    let cert = match X509::from_pem(&pem_data) {
        ...
        Err(e) => {
            issues.push(format!(
                "Invalid PEM certificate {path}: {e}"
            ));
            return json!({
                "path": path,
                "exists": true,
    ...
    if let Some(days) = days_until_expiry {
        if days < 0 {
            ...
            suggestions.push(format!(
                "Certificate {path} expires in {days} days"
            ));
        }
    }
...
fn inspect_key(
    ...
    match PKey::private_key_from_pem(&pem_data) {
        ...
        Err(e) => {
            ...
            issues.push(format!(
                "Invalid PEM private key {path}: {e}"
            ));
            json!({
                "path": path,
                "exists": true,
...
mod tests {
    ...
    fn generate_test_cert_and_key(
        ...
        let rsa =
            Rsa::generate(2048).expect("Failed to generate RSA key");
        let key = PKey::from_rsa(rsa)
            .expect("Failed to create PKey");

        let mut name_builder = X509NameBuilder::new()
            .expect("Failed to create X509NameBuilder");
        let _ = name_builder.append_entry_by_text(
            "CN",
            "test-keylimectl",
        );
        ...
        let mut builder = X509::builder()
            .expect("Failed to create X509 builder");
        ...
        let not_before = Asn1Time::days_from_now(0)
            .expect("Failed to create not_before");
        let not_after = Asn1Time::days_from_now(365)
            .expect("Failed to create not_after");
    ...
    fn test_inspect_certificate_valid() {
        ...
        tmpfile.write_all(&cert_pem).unwrap();
    ...
    fn test_inspect_certificate_invalid_pem() {
        let mut tmpfile =
            tempfile::NamedTempFile::new().unwrap();
        tmpfile.write_all(b"not a certificate").unwrap();
    ...
    fn test_inspect_key_not_found() {
        let mut issues = Vec::new();
        let info =
            inspect_key("/nonexistent/key.pem", &mut issues);
        assert_eq!(info["exists"], false);
        assert_eq!(issues.len(), 1);
    }
    ...
    fn test_inspect_key_valid() {
        ...
        let mut tmpfile =
            tempfile::NamedTempFile::new().unwrap();
        tmpfile.write_all(&key_pem).unwrap();

        let mut issues = Vec::new();
        let info = inspect_key(
            tmpfile.path().to_str().unwrap(),
            &mut issues,
        );
    ...
    fn test_verify_cert_key_pair_mismatched() {
        ...
        let mut key_file =
            tempfile::NamedTempFile::new().unwrap();
        key_file.write_all(&other_key_pem).unwrap();
