...
mod tests {
    ...
    fn test_build_version_info() {
        ...
        assert!(
            info["features"]["tpm-quote-validation"].is_boolean()
        );
    }
    ...
    fn test_build_config_file_info_defaults() {
        ...
        let searched = info["searched"].as_array().unwrap();
    ...
    fn test_build_effective_config() {
        ...
        assert_eq!(
            effective["verifier"]["ip"]["source"], "cli"
        );
        // verifier_port should be "default" (no override, default value)
        assert_eq!(
            effective["verifier"]["port"]["source"], "default"
        );
    }
}
