...
pub async fn execute(
    ...
    output.progress(format!(
        "Gathering diagnostics for agent {agent_id}"
    ));
    ...
    result["agent_direct"] =
        query_agent_direct(&result, output).await;

    Ok(result)
}

/// Query registrar for agent data.
async fn query_registrar(
    agent_id: &str,
    output: &OutputHandler,
) -> Value {
...
fn build_summary(result: &Value) -> Value {
    let registered =
        result["registrar"]["status"].as_str() == Some("found");
    let monitored =
        result["verifier"]["status"].as_str() == Some("found");
...
#[cfg(feature = "api-v2")]
fn extract_agent_connection(
    result: &Value,
) -> Option<(String, u16)> {
    let registrar_data = result
        .get("registrar")
        .and_then(|r| r.get("data"));
    let verifier_data = result
        .get("verifier")
...
mod tests {
    ...
    fn test_build_summary_both_found() {
        ...
        assert_eq!(
            summary["operational_state"], "Get Quote"
        );
    }
