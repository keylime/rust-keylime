...
fn resolve_config_path(
    ...
    match scope {
        ConfigScope::Local => Ok(PathBuf::from(
            ".keylimectl/config.toml",
        )),
        ConfigScope::User => {
...
fn run_interactive_wizard(
    ...
    let verifier_port: u16 = Input::new()
        ...
        .default(
            default_verifier_port.unwrap_or(defaults.verifier.port),
        )
        .interact_text()
        .map_err(|e| {
            KeylimectlError::Validation(format!(
    ...
    let registrar_port: u16 = Input::new()
        ...
        .default(
            default_registrar_port
                .unwrap_or(defaults.registrar.port),
        )
        .interact_text()
        .map_err(|e| {
            KeylimectlError::Validation(format!(
...
mod tests {
    ...
    fn test_build_non_interactive_config_defaults() {
        let config =
            build_non_interactive_config(None, None, None, None);
    ...
    fn test_resolve_config_path_local() {
        let path = resolve_config_path(&ConfigScope::Local).unwrap();
        assert_eq!(
            path,
            PathBuf::from(".keylimectl/config.toml")
        );
    }

    #[test]
    fn test_resolve_config_path_user() {
        let path = resolve_config_path(&ConfigScope::User).unwrap();
    ...
    fn test_resolve_config_path_system() {
        let path =
            resolve_config_path(&ConfigScope::System).unwrap();
        assert_eq!(
            path,
    ...
    fn test_write_config_file_overwrites_existing() {
        ...
        write_config_file(&config_path, &config1).unwrap();
        ...
        write_config_file(&config_path, &config2).unwrap();
