...
pub(super) async fn add_agent(
    ...
    let agent_ip =
        agent_ip.unwrap_or_else(|| "0.0.0.0".to_string());
...
fn extract_operational_state(data: &Value) -> Option<&str> {
    data.get("results")
        ...
        .and_then(|s| s.as_str())
        .or_else(|| {
            data.get("operational_state").and_then(|s| s.as_str())
        })
}
...
async fn poll_attestation_status(
    ...
    loop {
        ...
        match verifier_client.get_agent(agent_id).await {
            ...
            Ok(None) => {
                debug!(
                    "Agent not yet visible on verifier, waiting..."
                );
            }
            Err(e) => {
...
mod tests {
    ...
    fn test_attestation_state_classification() {
        ...
        let initial_states = ["Start", "Tenant Start", "Registered"];
        let failure_states =
            ["Failed", "Terminated", "Invalid Quote"];
        ...
        let progress_states =
            ["Get Quote", "Provide V", "Provide V (Retry)"];
