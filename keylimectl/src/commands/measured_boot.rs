...
//! # Command Types
//!
//! - [`MeasuredBootAction::Create`]: Create a new measured boot policy
...
//! let create_action = MeasuredBootAction::Create {
//!     name: "secure-boot-policy".to_string(),
//!     file: "/etc/keylime/policies/secure-boot.json".to_string(),
//! };
//!
//! let result = measured_boot::execute(&create_action, &config, &output).await?;
...
/// # Arguments
///
/// * `action` - The specific measured boot action to perform (Create, Show, Update, Delete, or List)
...
mod tests {
    ...
    mod action_variants {
        ...
        fn test_create_action() {
            let action = MeasuredBootAction::Create {
    ...
    mod policy_validation {
        ...
        fn test_valid_policy_structure() {
            ...
            assert_eq!(policy["components"].as_array().unwrap().len(), 1);
        }
        ...
        fn test_policy_with_different_pcrs() {
            ...
            let pcrs = policy["pcrs"].as_object().unwrap();
        ...
        fn test_policy_with_multiple_components() {
            ...
            let components = policy["components"].as_array().unwrap();
        ...
        fn test_policy_with_settings() {
            ...
            let settings = policy["settings"].as_object().unwrap();
    ...
    mod json_responses {
        ...
        fn test_success_response_structure() {
            let response = json!({
                "status": "success",
                "message": "Measured boot policy 'test-policy' created successfully",
        ...
        fn test_list_response_structure() {
            ...
            assert_eq!(
                response["results"]["policies"].as_array().unwrap().len(),
                3
            );
        }
