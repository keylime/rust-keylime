...
pub async fn execute(
    ...
    output.info(format!(
        "Verifying {evidence_type} attestation evidence"
    ));
    ...
    let response =
        client.verify_evidence(request_body).await?;
...
fn format_evidence_result(
    ...
    if valid {
        ...
        if let Some(failures) =
            results.get("failures").and_then(|f| {
                f.as_array()
            })
        {
...
mod tests {
    ...
    fn test_read_and_b64() {
        ...
        std::fs::write(tmp.path(), b"test data")
            .unwrap(); //#[allow_ci]
        let path =
    ...
    fn test_read_file_string() {
        let tmp =
            tempfile::NamedTempFile::new().unwrap(); //#[allow_ci]
        std::fs::write(tmp.path(), "text content")
            .unwrap(); //#[allow_ci]
        let path =
            tmp.path().to_string_lossy().to_string();

        let result =
            read_file_string(&path).unwrap(); //#[allow_ci]
