...
pub async fn execute(
    ...
    let _: Value =
        serde_json::from_slice(&policy_content)
            .map_err(|e| {
                KeylimectlError::validation(format!(
    ...
    if let Some(out_path) = output_file {
        let json_str =
            serde_json::to_string_pretty(&envelope_json)?;
        std::fs::write(out_path, &json_str).map_err(
            |e| {
                KeylimectlError::validation(format!(
        ...
        output.info(format!(
            "Signed policy written to {out_path}"
        ));
    } else {
...
fn sign_ecdsa(
    ...
    let signer = if let Some(kf) = keyfile {
        ...
        let key_path = keypath
            .unwrap_or("keylime-ecdsa-key.pem");
...
fn sign_x509(
    ...
    let signer = if let Some(kf) = keyfile {
        ...
        X509Signer::from_files(kf, cert_path).map_err(
            |e| {
                KeylimectlError::validation(format!(
        ...
        let cert_path = cert_outfile
            .unwrap_or("keylime-cert.pem");
        let s =
            X509Signer::generate(Some(cert_path))
                .map_err(|e| {
                    KeylimectlError::validation(format!(
        ...
        let key_path =
            keypath.unwrap_or("keylime-ecdsa-key.pem");
        s.save_private_key(key_path).map_err(|e| {
            KeylimectlError::validation(format!(
                "Failed to save key: {e}"
            ))
