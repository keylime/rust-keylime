...
async fn generate_runtime(
    ...
    let mut detected_algorithm: Option<String> =
        hash_alg.map(String::from);
    ...
    if let Some(ima_path) = ima_measurement_list {
        ...
        if path.exists() {
            output.info(format!(
                "Parsing IMA measurement list: {ima_path}"
            ));

            let ima_data =
                ima_parser::parse_ima_measurement_list(
                    path,
                    get_keyrings,
                    get_ima_buf,
                    ignored_keyrings,
            ...
            log::debug!(
                "IMA measurement list not found: {ima_path}"
            );
        }
    }
    ...
    if let Some(rootfs_path) = rootfs {
        let algorithm = detected_algorithm
            .as_deref()
            .unwrap_or("sha256");
    ...
    if let Some(excludelist_path) = excludelist {
        ...
        output.info(format!(
            "Parsing exclude list: {excludelist_path}"
        ));
...
fn generate_measured_boot(
    ...
    output.info(format!(
        "Parsing UEFI event log: {eventlog_file}"
    ));

    let include_secureboot = !without_secureboot;
    let policy =
        measured_boot_gen::generate_from_eventlog(
            path,
            include_secureboot,
        )?;
...
fn generate_tpm(
    ...
    let pcr_file = pcr_file.ok_or_else(|| {
        CommandError::from(
            crate::commands::error::PolicyGenerationError::Output {
                path: "<pcr-file>".into(),
                reason: "Either --pcr-file or --from-tpm is required".to_string(),
            },
        )
    })?;
    ...
    output.info(format!(
        "Reading PCR values from: {pcr_file}"
    ));
    output.info(format!(
        "PCR indices: {:?}",
        pcr_indices
...
fn load_base_policy(
    path: &str,
) -> Result<RuntimePolicy, CommandError> {
    let content =
