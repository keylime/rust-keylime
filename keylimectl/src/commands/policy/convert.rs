...
pub async fn execute(
    ...
    let mut policy =
        conversion::convert_allowlist_file(Path::new(
            file,
        ))?;

    output.info(format!(
        "Converted {} file entries",
        policy.digest_count()
    ));
    ...
    if let Some(excl_path) = excludelist {
        let excludes =
            ima_parser::parse_excludelist(Path::new(
                excl_path,
            ))?;
        conversion::merge_excludelist(
            &mut policy,
            &excludes,
        );
        output.info(format!(
    ...
    if let Some(key_path) = verification_keys {
        conversion::add_verification_keys(
            &mut policy, key_path,
        )?;
        output.info(format!(
            "Added verification keys from '{key_path}'"
        ));
    }
    ...
    output.info(format!(
        "Runtime policy written to {output_file}"
    ));
