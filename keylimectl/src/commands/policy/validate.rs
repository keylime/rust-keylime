...
use crate::policy_tools::dsse::{
    self, ecdsa_backend::EcdsaVerifier,
    x509_backend::X509Verifier, DsseEnvelope, Verifier,
};
use crate::policy_tools::validation::{
...
pub async fn execute(
    ...
    let detected_type = policy_type
        .or_else(|| {
            validation::detect_policy_type(&json_value)
        });
    ...
    output.info(format!(
        "Validating {policy_type_str} policy from {file}"
    ));
    ...
    let result = match policy_type_str {
        ...
        "measured-boot" => {
            ...
            validation::validate_measured_boot_policy(
                &policy,
            )
        }
        "tpm" => {
    ...
    if let Some(key) = signature_key {
        let sig_result =
            verify_signature(file, key, output).await?;
...
fn format_validation_result(
    ...
    if result.valid {
        output.info(format!(
            "Policy validation passed ({policy_type})"
        ));
    } else {
        output.info(format!(
            "Policy validation failed ({policy_type})"
        ));
    }
...
pub async fn verify_signature(
    ...
    let envelope: DsseEnvelope =
        serde_json::from_str(&envelope_content)
            .map_err(|e| {
                KeylimectlError::validation(format!(
    ...
    let key_str =
        std::str::from_utf8(&key_content).unwrap_or("");

    // Determine if this is a certificate or public key
    let verifiers: Vec<(
        &str,
        Box<dyn Verifier>,
    )> = if key_str.contains("BEGIN CERTIFICATE") {
        vec![(
            "x509",
            Box::new(
                X509Verifier::from_cert_pem(
                    &key_content,
                )
                .map_err(|e| {
    ...
    let verifier_refs: Vec<(&str, &dyn Verifier)> =
        verifiers
            .iter()
