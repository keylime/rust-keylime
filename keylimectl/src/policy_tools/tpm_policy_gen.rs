...
pub fn generate_from_file(
        ...
        content.lines().enumerate()
    {
        ...
        let (index, value) = if parts.len() == 2 {
            let idx =
                parts[0].trim().parse::<u32>().map_err(
    ...
    if pcrs.is_empty() {
        return Err(PolicyGenerationError::Output {
            path: path.to_path_buf(),
            reason:
                "No PCR values found for the requested indices"
                    .to_string(),
        });
    }
...
mod tests {
    ...
    fn test_generate_from_file_filter_indices() {
        ...
        assert!(
            policy.pcr_values.contains_key("0")
        );
        assert!(
            policy.pcr_values.contains_key("7")
        );
        assert!(
            !policy.pcr_values.contains_key("1")
        );
    }
    ...
    fn test_parse_pcr_indices_out_of_range() {
        assert!(
            parse_pcr_indices("0,1,25").is_err()
        );
    }
    ...
    fn test_parse_pcr_indices_invalid() {
        assert!(
            parse_pcr_indices("0,abc,7").is_err()
        );
    }
}
