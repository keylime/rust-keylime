...
/// Validate that an href from repository XML is a safe relative path.
///
/// Rejects absolute URLs, path traversal, query strings, fragments,
/// and null bytes to prevent SSRF via injected hrefs.
fn validate_relative_href(href: &str) -> Result<(), PolicyGenerationError> {
    if href.contains("://") {
        return Err(PolicyGenerationError::RpmParse {
            path: PathBuf::from("<repodata>"),
            reason: format!(
                "Absolute URL in repository href is not allowed: {href}"
            ),
        });
    }
    if href.contains("..") {
        return Err(PolicyGenerationError::RpmParse {
            path: PathBuf::from("<repodata>"),
            reason: format!(
                "Path traversal in repository href is not allowed: {href}"
            ),
        });
    }
    if href.contains('?') || href.contains('#') || href.contains('\0') {
        return Err(PolicyGenerationError::RpmParse {
            path: PathBuf::from("<repodata>"),
            reason: format!(
                "Query string, fragment, or null byte in repository href is not allowed: {href}"
            ),
        });
    }
    Ok(())
...
pub async fn analyze_remote_repo(
    ...
    match fetch_optional_bytes(&sig_url).await? {
        ...
        None => {
            if allow_unsigned {
                log::warn!(
                    "Unsigned repository metadata (no repomd.xml.asc found); \
                     continuing because --allow-unsigned-repo was specified"
                );
            } else {
                return Err(PolicyGenerationError::GpgVerification {
                    path: PathBuf::from(&repomd_url),
                    reason: "Repository metadata is unsigned (no repomd.xml.asc). \
                             Use --allow-unsigned-repo to proceed anyway, or \
                             provide a GPG key with --gpg-key."
                        .to_string(),
                });
            }
        }
    ...
    {
        validate_relative_href(&filelists_href)?;
    ...
    validate_relative_href(&primary_href)?;
