...
impl EcdsaSigner {
    ...
    pub fn from_pem_file(
        ...
        if private_key.ec_key().is_err() {
            return Err(
                "Key is not an EC key".to_string()
            );
        }
...
impl EcdsaVerifier {
    ...
    pub fn from_pem(
        ...
        let public_key =
            PKey::public_key_from_pem(pem).map_err(|e| {
...
mod tests {
    ...
    fn test_sign_verify_roundtrip() {
        let signer = EcdsaSigner::generate().unwrap(); //#[allow_ci]
        let verifier =
            EcdsaVerifier::from_signer(&signer).unwrap(); //#[allow_ci]
    ...
    fn test_verify_wrong_message() {
        ...
        let signature =
            signer.sign(b"original message").unwrap(); //#[allow_ci]

        assert!(
            !verifier.verify(b"different message", &signature)
        );
    }
    ...
    fn test_verify_wrong_key() {
        ...
        let verifier2 =
            EcdsaVerifier::from_signer(&signer2).unwrap(); //#[allow_ci]

        let message = b"test message";
        let signature =
            signer1.sign(message).unwrap(); //#[allow_ci]
    ...
    fn test_save_and_load_key() {
        ...
        let loaded =
            EcdsaSigner::from_pem_file(&path).unwrap(); //#[allow_ci]

        // Verify with the loaded key's public key
        let verifier =
            EcdsaVerifier::from_signer(&loaded).unwrap(); //#[allow_ci]
