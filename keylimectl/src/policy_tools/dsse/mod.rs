...
pub const KEYLIME_PAYLOAD_TYPE: &str =
...
pub fn pae(payload_type: &str, payload: &[u8]) -> Vec<u8> {
    ...
    result.extend_from_slice(
        pt_bytes.len().to_string().as_bytes(),
    );
    result.push(b' ');
...
mod tests {
    ...
    fn test_pae_construction() {
        ...
        let expected = b"DSSEv1 28 application/vnd.keylime+json 11 hello world";
    ...
    fn test_envelope_serialization() {
        ...
        let json =
            serde_json::to_string(&envelope).unwrap(); //#[allow_ci]
    ...
    fn test_sign_and_verify_roundtrip() {
        ...
        let signer =
            ecdsa_backend::EcdsaSigner::generate().unwrap(); //#[allow_ci]
        let verifier =
            ecdsa_backend::EcdsaVerifier::from_signer(
                &signer,
            )
            .unwrap(); //#[allow_ci]
    ...
    fn test_verify_invalid_signature() {
        ...
        let verifier_ref: &dyn Verifier =
            &other_verifier;
        let result = verify_envelope(
            &envelope,
            &[("wrong-key", verifier_ref)],
        );
