...
pub trait Verifier {
    ...
    ///
    /// Returns `Ok(true)` when the signature is valid, `Ok(false)` when the
    /// signature does not match, and `Err` when the verification operation
    /// itself fails (e.g., due to an internal cryptographic error).
    fn verify(
        &self,
        message: &[u8],
        signature: &[u8],
    ...
    /// Return the key identifier for this verifier.
    ///
    /// This must match the keyid produced by the corresponding [`Signer`]
    /// so that [`verify_envelope`] can bind each signature to the correct
    /// verifier without trying every verifier against every signature.
    fn keyid(&self) -> String;
}
...
///
/// Each signature in the envelope is matched to verifiers by keyid.  When
/// the signature's keyid is non-empty, only verifiers whose [`Verifier::keyid`]
/// matches are tried; this prevents a verifier from being used against a
/// signature it was never intended to verify.  An empty keyid falls back to
/// trying every verifier for backward compatibility.
pub fn verify_envelope(
    envelope: &DsseEnvelope,
    verifiers: &[(&str, &dyn Verifier)],
) -> Result<VerifiedPayload, DsseError> {
    let payload = Base64.decode(&envelope.payload).map_err(|e| {
        DsseError::InvalidEnvelope {
    ...
    let mut by_keyid: HashMap<String, Vec<(&str, &dyn Verifier)>> =
        HashMap::new();
    for (name, verifier) in verifiers {
        by_keyid
            .entry(verifier.keyid())
            .or_default()
            .push((name, *verifier));
    }

    for sig_entry in &envelope.signatures {
        let sig_bytes = Base64.decode(&sig_entry.sig).map_err(|e| {
            DsseError::InvalidEnvelope {
                reason: format!("Invalid base64 signature: {e}"),
            }
        })?;

        let candidates: &[(&str, &dyn Verifier)] =
            if sig_entry.keyid.is_empty() {
                verifiers
            } else {
                by_keyid
                    .get(&sig_entry.keyid)
    ...
    if recognized_signers.is_empty() {
        return Err(DsseError::VerificationFailed {
            reason: "No valid signature found".to_string(),
        });
    }
...
mod tests {
    ...
    #[test]
    fn test_sign_and_verify_x509_roundtrip() {
        let signer = x509_backend::X509Signer::generate(None).unwrap(); //#[allow_ci]
        let verifier =
            x509_backend::X509Verifier::from_signer(&signer).unwrap(); //#[allow_ci]
        ...
        let verifier_ref: &dyn Verifier = &verifier;
        let result =
            verify_envelope(&envelope, &[("x509", verifier_ref)]).unwrap(); //#[allow_ci]

        assert_eq!(result.payload, payload);
        assert_eq!(result.payload_type, KEYLIME_PAYLOAD_TYPE);
        assert_eq!(result.recognized_signers, vec!["x509"]);
    }
}
