...
impl X509Signer {
    ...
    pub fn generate(cert_path: Option<&str>) -> Result<Self, DsseError> {
        let group =
        ...
        let certificate_pem =
            build_self_signed_cert(&private_key, "keylimectl", 365)?;

        if let Some(path) = cert_path {
            std::fs::write(path, &certificate_pem).map_err(|e| {
                DsseError::KeyError {
                    reason: format!("Failed to write certificate: {e}"),
                }
            })?;
        }
    ...
    pub fn from_files(
        ...
        let key_pem =
            std::fs::read(key_path).map_err(|e| DsseError::KeyError {
                reason: format!("Failed to read key file: {e}"),
            })?;
        let cert_pem =
            std::fs::read(cert_path).map_err(|e| DsseError::KeyError {
        ...
        // Parse the certificate and verify it matches the private key.
        let cert =
            X509::from_pem(&cert_pem).map_err(|e| DsseError::KeyError {
                reason: format!("Failed to parse certificate: {e}"),
            })?;

        let cert_public_key =
            cert.public_key().map_err(|e| DsseError::KeyError {
                reason: format!(
...
impl X509Verifier {
    ...
    ///
    /// Returns an error if the certificate is expired or not yet valid.
    pub fn from_cert_pem(pem: &[u8]) -> Result<Self, DsseError> {
        let cert = X509::from_pem(pem).map_err(|e| DsseError::KeyError {
            reason: format!("Failed to parse certificate: {e}"),
        })?;

        use std::cmp::Ordering;
        let now =
            Asn1Time::days_from_now(0).map_err(|e| DsseError::KeyError {
                reason: format!("Failed to obtain current time: {e}"),
            })?;
        if cert.not_before().compare(&now).map_err(|e| {
            DsseError::KeyError {
                reason: format!(
                    "Failed to compare certificate validity dates: {e}"
                ),
            }
        })? == Ordering::Greater
        {
        ...
        if cert.not_after().compare(&now).map_err(|e| {
            DsseError::KeyError {
                reason: format!(
                    "Failed to compare certificate validity dates: {e}"
                ),
        ...
        let public_key =
            cert.public_key().map_err(|e| DsseError::KeyError {
                reason: format!(
                    "Failed to extract public key from cert: {e}"
                ),
            })?;
    ...
    pub fn from_cert_file(path: &str) -> Result<Self, DsseError> {
    ...
    #[allow(dead_code)] // Used only in tests; see test_sign_and_verify_x509_roundtrip in dsse/mod.rs
    pub fn from_signer(signer: &X509Signer) -> Result<Self, DsseError> {
...
fn build_self_signed_cert(
    ...
    let mut builder = openssl::x509::X509Builder::new().map_err(|e| {
        DsseError::KeyError {
            reason: format!("Failed to create X509 builder: {e}"),
        }
    })?;

    // Version 3
    builder.set_version(2).map_err(|e| DsseError::KeyError {
        reason: format!("Failed to set version: {e}"),
    })?;

    // Serial number: random 128-bit value, MSB cleared to ensure positive sign.
    let mut serial_bytes = [0u8; 16];
    openssl::rand::rand_bytes(&mut serial_bytes).map_err(|e| {
        DsseError::KeyError {
            reason: format!("Failed to generate random serial bytes: {e}"),
        }
    })?;
    serial_bytes[0] &= 0x7F;
    let serial = BigNum::from_slice(&serial_bytes)
        ...
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to create serial: {e}"),
        })?;
    builder
        .set_serial_number(&serial)
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to set serial: {e}"),
        })?;

    // Subject and issuer (same for self-signed)
    let mut name_builder =
        X509NameBuilder::new().map_err(|e| DsseError::KeyError {
            reason: format!("Failed to create name builder: {e}"),
        })?;
    name_builder
        .append_entry_by_text("CN", subject_name)
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to add CN: {e}"),
        })?;
    ...
    builder
        ...
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to set subject: {e}"),
        })?;
    builder
        .set_issuer_name(&name)
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to set issuer: {e}"),
        })?;

    // Validity period
    let not_before =
        Asn1Time::days_from_now(0).map_err(|e| DsseError::KeyError {
            reason: format!("Failed to create not_before: {e}"),
        })?;
    let not_after =
        Asn1Time::days_from_now(expiration_days).map_err(|e| {
            DsseError::KeyError {
                reason: format!("Failed to create not_after: {e}"),
            }
        })?;
    builder
        .set_not_before(&not_before)
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to set not_before: {e}"),
        })?;
    builder
        .set_not_after(&not_after)
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to set not_after: {e}"),
        })?;
    ...
    builder
        ...
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to set public key: {e}"),
        })?;
    ...
    let basic_constraints =
        BasicConstraints::new()
            .build()
            .map_err(|e| DsseError::KeyError {
                reason: format!("Failed to build basic constraints: {e}"),
            })?;
    builder.append_extension(basic_constraints).map_err(|e| {
        DsseError::KeyError {
            reason: format!("Failed to append basic constraints: {e}"),
        }
    })?;
    ...
    let san = SubjectAlternativeName::new()
        ...
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to build SAN: {e}"),
        })?;
    builder
        .append_extension(san)
        .map_err(|e| DsseError::KeyError {
            reason: format!("Failed to append SAN: {e}"),
        })?;
    ...
    cert.to_pem().map_err(|e| DsseError::KeyError {
        reason: format!("Failed to encode certificate: {e}"),
    })
}
...
mod tests {
    ...
    fn test_verify_wrong_message() {
        ...
        assert!(!verifier.verify(b"different", &signature).unwrap()); //#[allow_ci]
    }
    ...
    #[test]
    fn test_from_files_rejects_mismatched_key_and_cert() {
        let signer_a = X509Signer::generate(None).unwrap(); //#[allow_ci]
        let signer_b = X509Signer::generate(None).unwrap(); //#[allow_ci]
        ...
        signer_a.save_private_key(&key_path).unwrap(); //#[allow_ci]
        std::fs::write(&cert_path, signer_b.certificate_pem()).unwrap(); //#[allow_ci]

        let result = X509Signer::from_files(&key_path, &cert_path);
    ...
    #[test]
    fn test_serial_is_random() {
        let signer_a = X509Signer::generate(None).unwrap(); //#[allow_ci]
        let signer_b = X509Signer::generate(None).unwrap(); //#[allow_ci]

        let cert_a = X509::from_pem(signer_a.certificate_pem()).unwrap(); //#[allow_ci]
        let cert_b = X509::from_pem(signer_b.certificate_pem()).unwrap(); //#[allow_ci]
        ...
        assert_ne!(serial_a, serial_b, "serial numbers must differ");
    }
