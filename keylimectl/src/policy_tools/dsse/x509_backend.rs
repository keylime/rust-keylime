...
use openssl::x509::extension::{
    BasicConstraints, SubjectAlternativeName,
};
...
impl X509Signer {
    ...
    pub fn generate(
        ...
        let ec_key = EcKey::generate(&group).map_err(|e| {
            format!("Failed to generate EC key: {e}")
        })?;
        let private_key =
            PKey::from_ec_key(ec_key).map_err(|e| {
                format!("Failed to wrap EC key: {e}")
            })?;

        let certificate_pem = build_self_signed_cert(
            &private_key,
            "keylimectl",
            30,
        )?;
    ...
    pub fn from_files(
        ...
        let _cert = X509::from_pem(&cert_pem).map_err(|e| {
...
impl Signer for X509Signer {
    fn sign(
        ...
        let mut signer = sign::Signer::new(
        ...
        signer.update(message).map_err(|e| {
...
fn build_self_signed_cert(
    ...
    let mut builder =
        openssl::x509::X509Builder::new().map_err(|e| {
            format!("Failed to create X509 builder: {e}")
        })?;

    // Version 3
    builder.set_version(2).map_err(|e| {
        format!("Failed to set version: {e}")
    })?;
    ...
    let serial = BigNum::from_u32(1)
        ...
        .map_err(|e| {
            format!("Failed to create serial: {e}")
        })?;
    builder.set_serial_number(&serial).map_err(|e| {
        format!("Failed to set serial: {e}")
    })?;

    // Subject and issuer (same for self-signed)
    let mut name_builder =
        X509NameBuilder::new().map_err(|e| {
            format!("Failed to create name builder: {e}")
        })?;
    name_builder
    ...
    builder.set_subject_name(&name).map_err(|e| {
        format!("Failed to set subject: {e}")
    })?;
    builder.set_issuer_name(&name).map_err(|e| {
        format!("Failed to set issuer: {e}")
    })?;

    // Validity period
    let not_before =
        Asn1Time::days_from_now(0).map_err(|e| {
            format!("Failed to create not_before: {e}")
        })?;
    let not_after =
        Asn1Time::days_from_now(expiration_days)
            .map_err(|e| {
                format!("Failed to create not_after: {e}")
            })?;
    builder.set_not_before(&not_before).map_err(|e| {
        format!("Failed to set not_before: {e}")
    })?;
    builder.set_not_after(&not_after).map_err(|e| {
        format!("Failed to set not_after: {e}")
    })?;

    // Public key
    builder.set_pubkey(private_key).map_err(|e| {
        format!("Failed to set public key: {e}")
    })?;
    ...
    let basic_constraints = BasicConstraints::new()
        ...
        .map_err(|e| {
            format!(
                "Failed to build basic constraints: {e}"
            )
        })?;
    builder
        ...
        .map_err(|e| {
            format!(
                "Failed to append basic constraints: {e}"
            )
        })?;
    ...
    let san = SubjectAlternativeName::new()
        ...
        .map_err(|e| {
            format!("Failed to build SAN: {e}")
        })?;
    builder.append_extension(san).map_err(|e| {
        format!("Failed to append SAN: {e}")
    })?;
...
mod tests {
    ...
    fn test_generate_signer() {
        let signer =
            X509Signer::generate(None).unwrap(); //#[allow_ci]
    ...
    fn test_sign_verify_roundtrip() {
        let signer =
            X509Signer::generate(None).unwrap(); //#[allow_ci]
        let verifier =
            X509Verifier::from_signer(&signer).unwrap(); //#[allow_ci]

        let message = b"test message";
        let signature =
            signer.sign(message).unwrap(); //#[allow_ci]
    ...
    fn test_verify_wrong_message() {
        ...
        let signature =
            signer.sign(b"original").unwrap(); //#[allow_ci]

        assert!(
            !verifier.verify(b"different", &signature)
        );
    }
    ...
    fn test_save_and_load() {
        ...
        let key_file =
            tempfile::NamedTempFile::new().unwrap(); //#[allow_ci]
        let cert_file =
            tempfile::NamedTempFile::new().unwrap(); //#[allow_ci]
        ...
        std::fs::write(&cert_path, signer.certificate_pem())
            .unwrap(); //#[allow_ci]

        let loaded =
            X509Signer::from_files(&key_path, &cert_path)
                .unwrap(); //#[allow_ci]

        let verifier =
            X509Verifier::from_signer(&loaded).unwrap(); //#[allow_ci]
    ...
    fn test_certificate_saved_to_file() {
        ...
        let _signer =
            X509Signer::generate(Some(&cert_path)).unwrap(); //#[allow_ci]

        let cert_contents =
            std::fs::read_to_string(&cert_path).unwrap(); //#[allow_ci]
        assert!(
