...
impl TpmPolicy {
    ...
    pub fn calculate_mask(indices: &[u32]) -> String {
        let mask: u32 =
            indices.iter().fold(0u32, |acc, &i| acc | (1 << i));
        format!("0x{mask:x}")
    }
    ...
    pub fn parse_mask(mask: &str) -> Result<Vec<u32>, String> {
        ...
        let value = u32::from_str_radix(hex_str, 16)
            .map_err(|e| {
                format!("Invalid PCR mask '{mask}': {e}")
            })?;
...
mod tests {
    ...
    fn test_calculate_mask() {
        assert_eq!(
            TpmPolicy::calculate_mask(&[0, 1, 2, 7]),
            "0x87"
        );
        assert_eq!(
            TpmPolicy::calculate_mask(&[]),
            "0x0"
        );
        assert_eq!(
            TpmPolicy::calculate_mask(&[0]),
            "0x1"
        );
    }
    ...
    fn test_parse_mask() {
        assert_eq!(
            TpmPolicy::parse_mask("0x87").unwrap(),
            vec![0, 1, 2, 7]
        );
        assert_eq!(
            TpmPolicy::parse_mask("0x0").unwrap(),
            Vec::<u32>::new()
        );
        assert_eq!(
            TpmPolicy::parse_mask("0x1").unwrap(),
            vec![0]
        );
        // Without prefix
        assert_eq!(
            TpmPolicy::parse_mask("ff").unwrap(),
            vec![0, 1, 2, 3, 4, 5, 6, 7]
        );
    }
    ...
    fn test_mask_roundtrip() {
        ...
        let parsed = TpmPolicy::parse_mask(&mask).unwrap();
