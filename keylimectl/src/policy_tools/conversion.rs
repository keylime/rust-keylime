...
pub fn convert_json_allowlist(
    ...
    let digests = ima_parser::parse_json_allowlist_value(
        json,
    )?;
...
pub fn convert_flat_allowlist(
    ...
    let digests =
        ima_parser::parse_flat_allowlist_str(text)?;
...
pub fn add_verification_keys(
    ...
    let key_content =
        std::fs::read_to_string(key_path).map_err(
            |e| PolicyGenerationError::Output {
                path: key_path.into(),
                reason: format!(
                    "Failed to read verification key: {e}"
                ),
            },
        )?;
...
mod tests {
    ...
    fn test_convert_json_allowlist_hashes_key() {
        ...
        let policy =
            convert_json_allowlist(&json).unwrap(); //#[allow_ci]
        assert_eq!(policy.digest_count(), 2);
        assert_eq!(
            policy.digests["/usr/bin/bash"],
            vec!["sha256:aabbccdd"]
        );
        assert_eq!(
    ...
    fn test_convert_flat_allowlist() {
        ...
        let policy =
            convert_flat_allowlist(text).unwrap(); //#[allow_ci]
        assert_eq!(policy.digest_count(), 2);
        assert_eq!(
            policy.digests["/usr/bin/bash"],
            vec!["sha256:aabb1122"]
    ...
    fn test_auto_detect_json() {
        let input =
            br#"{"hashes": {"/test": ["sha256:abcd"]}}"#;

        let policy =
            convert_allowlist(input).unwrap(); //#[allow_ci]
        assert_eq!(policy.digest_count(), 1);
    }
    ...
    fn test_add_verification_keys() {
        ...
        std::fs::write(tmp.path(), "PEM KEY DATA")
            .unwrap(); //#[allow_ci]
        let path =
            tmp.path().to_string_lossy().to_string();

        add_verification_keys(&mut policy, &path)
            .unwrap(); //#[allow_ci]
        assert_eq!(
    ...
    fn test_convert_nonexistent_file() {
        let result = convert_allowlist_file(Path::new(
            "/nonexistent/file",
        ));
