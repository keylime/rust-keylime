...
pub fn validate_runtime_policy(
    ...
    // Check digest format: "algorithm:hex_value"
    for (path, digests) in &policy.digests {
        if digests.is_empty() {
            result.add_warning(
                "empty_digests",
                format!(
                    "Path '{path}' has no digests"
                ),
            );
            continue;
        }
    ...
    for (name, digests) in &policy.ima_buf {
        if name.is_empty() {
            result.add_error(
                "empty_ima_buf_name",
                "IMA-buf entry name must not be empty"
                    .to_string(),
            );
        }
        for digest in digests {
            validate_digest_string(
                digest, name, &mut result,
            );
        }
    }
...
pub fn validate_measured_boot_policy(
    ...
    for sig in &policy.pk {
        if sig.signature_owner.is_empty() {
            result.add_error(
                "empty_pk_owner",
                "PK signature owner must not be empty"
                    .to_string(),
            );
        }
        if sig.signature_data.is_empty() {
            result.add_error(
                "empty_pk_data",
                "PK signature data must not be empty"
                    .to_string(),
            );
        }
    }
    ...
    for sig in &policy.kek {
        if sig.signature_owner.is_empty() {
            result.add_error(
                "empty_kek_owner",
                "KEK signature owner must not be empty"
                    .to_string(),
            );
        }
    }
    ...
    for sig in &policy.db {
        if sig.signature_owner.is_empty() {
            result.add_error(
                "empty_db_owner",
                "DB signature owner must not be empty"
                    .to_string(),
            );
        }
    }
    ...
    if policy.kernels.is_empty() {
        result.add_warning(
            "no_kernels",
            "No kernel boot chain entries defined"
                .to_string(),
        );
    }
    ...
    for (i, kernel) in policy.kernels.iter().enumerate() {
            ...
            && kernel.initrd_plain_sha256.is_none()
        {
            result.add_warning(
                "kernel_no_hashes",
                format!(
                    "Kernel entry {i} has no digest values"
                ),
            );
        }
    }
            ...
            for idx in &indices {
                ...
                if !policy.pcr_values.contains_key(&key)
                {
                    result.add_error(
                        "missing_pcr_value",
                        format!(
                            "PCR {idx} is set in mask but has no value"
                        ),
                    );
                }
            }
            ...
            for key in policy.pcr_values.keys() {
                if let Ok(idx) = key.parse::<u32>() {
                    ...
                    result.add_error(
                        "invalid_pcr_key",
                        format!(
                            "PCR key '{key}' is not a valid integer"
                        ),
                    );
                }
            }
    ...
    for (key, value) in &policy.pcr_values {
            ...
            .all(|c| c.is_ascii_hexdigit())
        {
            result.add_error(
                "invalid_pcr_hex",
                format!(
                    "PCR {key} value is not valid hex"
                ),
            );
        }
    }
...
/// Validate a digest string in "algorithm:hex_value" format.
    ...
    match digest.split_once(':') {
        Some((alg, hex_value)) => {
            if alg.is_empty() {
                result.add_error(
                    "empty_algorithm",
                    format!(
                        "Digest for '{context}' has empty algorithm name: '{digest}'"
                    ),
                );
                return;
            }
            if hex_value.is_empty() {
                result.add_error(
                    "empty_hex_value",
                    format!(
                        "Digest for '{context}' has empty hex value: '{digest}'"
                    ),
                );
                return;
            }
                ...
                .all(|c| c.is_ascii_hexdigit())
            {
                result.add_error(
                    "invalid_hex",
                    format!(
                        "Digest for '{context}' has invalid hex value: '{digest}'"
                    ),
                );
                return;
            }

            // Check known algorithm digest lengths
                    ...
                    .find(|(name, _)| *name == alg)
            {
                if hex_value.len() != *expected_len {
                    result.add_warning(
                        ...
                        format!(
                            "Digest for '{context}' has unexpected length {} for {alg} (expected {expected_len}): '{digest}'",
                            hex_value.len()
                        ),
                    );
                }
        ...
        None => {
            result.add_error(
                ...
                format!(
                    "Digest for '{context}' missing algorithm prefix (expected 'algorithm:hex'): '{digest}'"
                ),
            );
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_invalid_digest_format_no_colon() {
        let mut policy = RuntimePolicy::new();
        policy.add_digest(
            "/usr/bin/bash".to_string(),
            "no_colon_digest".to_string(),
        );
    ...
    #[test]
    fn test_invalid_digest_format_bad_hex() {
        let mut policy = RuntimePolicy::new();
        policy.add_digest(
            "/usr/bin/bash".to_string(),
            "sha256:not_hex_value!".to_string(),
        );
    ...
    fn test_empty_keyring_name() {
        ...
        assert!(result
            .errors
            .iter()
            .any(|e| e.code == "empty_keyring_name"));
    }
    ...
    fn test_unknown_hash_alg_warning() {
        ...
        assert!(result
            .warnings
            .iter()
            .any(|e| e.code == "unknown_hash_alg"));
    }

    #[test]
    fn test_digest_length_mismatch_warning() {
        let mut policy = RuntimePolicy::new();
        // SHA-256 should be 64 hex chars, provide only 8
        policy.add_digest(
    ...
    fn test_tpm_missing_pcr_value() {
        ...
        let _ = policy.pcr_values.insert(
            "0".to_string(),
            "aabb".to_string(),
        );
        ...
        assert!(result
            .errors
            .iter()
            .any(|e| e.code == "missing_pcr_value"));
    }
    ...
    fn test_valid_measured_boot_policy() {
        ...
        policy.kernels.push(KernelEntry {
            shim_authcode_sha256: Some(
                "0xshimhash".to_string(),
            ),
            grub_authcode_sha256: None,
            kernel_authcode_sha256: Some(
                "0xkernhash".to_string(),
            ),
        ...
        let result =
            validate_measured_boot_policy(&policy);
        assert!(result.valid, "Errors: {:?}", result.errors);
    }
