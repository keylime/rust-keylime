...
///
/// Returns the digest as `"algorithm:hex_value"` (e.g., `"sha256:abcdef..."`).
pub fn calculate_file_digest(
    ...
    let digest = hasher.finish().map_err(|e| {
        PolicyGenerationError::Digest {
            path: path.to_path_buf(),
    ...
    Ok(format!("{}:{}", algorithm, hex::encode(digest)))
}
...
mod tests {
    ...
    fn test_calculate_file_digest_sha256() {
        ...
        let result =
            calculate_file_digest(f.path(), "sha256").unwrap(); //#[allow_ci]
    ...
    fn test_calculate_file_digest_sha1() {
        ...
        let result =
            calculate_file_digest(f.path(), "sha1").unwrap(); //#[allow_ci]

        assert!(result.starts_with("sha1:"));
    ...
    fn test_unsupported_algorithm() {
        ...
        let result =
            calculate_file_digest(f.path(), "md5");
    ...
    fn test_nonexistent_file() {
        let result = calculate_file_digest(
