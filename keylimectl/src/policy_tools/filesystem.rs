...
pub fn scan_filesystem(
    ...
    let skip_set: Vec<PathBuf> = skip_paths
        .iter()
        .map(PathBuf::from)
        .collect();
...
pub fn detect_non_root_mounts() -> Result<Vec<String>, PolicyGenerationError> {
    let content =
        std::fs::read_to_string("/proc/mounts").map_err(|e| {
            PolicyGenerationError::FilesystemScan {
...
mod tests {
    ...
    fn test_scan_filesystem_basic() {
        ...
        let result =
            scan_filesystem(root, &[], "sha256").unwrap(); //#[allow_ci]
        ...
        assert!(result["/file1.txt"][0].starts_with("sha256:"));
    }
    ...
    fn test_scan_filesystem_with_subdirs() {
        ...
        fs::write(root.join("usr/bin/bash"), "bash content")
            .unwrap(); //#[allow_ci]
        fs::write(root.join("usr/bin/ls"), "ls content")
            .unwrap(); //#[allow_ci]
    ...
    fn test_scan_filesystem_skip_paths() {
        ...
        fs::write(root.join("include/file.txt"), "include")
            .unwrap(); //#[allow_ci]
        fs::write(root.join("exclude/file.txt"), "exclude")
            .unwrap(); //#[allow_ci]

        let skip = vec![
            root.join("exclude").to_string_lossy().to_string()
        ];
        let result =
            scan_filesystem(root, &skip, "sha256").unwrap(); //#[allow_ci]
    ...
    fn test_scan_filesystem_empty_dir() {
        ...
        let result =
            scan_filesystem(dir.path(), &[], "sha256").unwrap(); //#[allow_ci]
