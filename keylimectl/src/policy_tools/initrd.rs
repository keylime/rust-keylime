...
const MAX_INITRD_DECOMPRESSED_SIZE: u64 = 1_073_741_824; // 1 GB
...
fn decompress(
    ...
    match format {
        Compression::Gzip => {
            ...
            let mut limited = decoder.take(MAX_INITRD_DECOMPRESSED_SIZE);
        ...
        Compression::Zstd => {
            let decoder = zstd::Decoder::new(data).map_err(|e| {
                PolicyGenerationError::Output {
                    path: "<initrd>".into(),
                    reason: format!("Zstd decoder init failed: {e}"),
                }
            })?;
            let mut limited = decoder.take(MAX_INITRD_DECOMPRESSED_SIZE);
            let mut decompressed = Vec::new();
...
fn extract_cpio_digests(
    ...
    loop {
        ...
        let filesize_raw =
            parse_hex_field(data, pos + CPIO_FILESIZE_OFFSET).unwrap_or(0);
        let namesize_raw =
            parse_hex_field(data, pos + CPIO_NAMESIZE_OFFSET).unwrap_or(0);
        let Some(filesize) = usize::try_from(filesize_raw).ok() else {
            break;
        };
        let Some(namesize) = usize::try_from(namesize_raw).ok() else {
            break;
        };
