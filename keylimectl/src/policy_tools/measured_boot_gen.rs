...
pub fn generate_from_eventlog(
    ...
    let handler = UefiLogHandler::new(path_str)
        .map_err(|e| {
            PolicyGenerationError::Output {
                path: path.to_path_buf(),
                reason: format!(
                    "Failed to parse UEFI event log: {e}"
                ),
            }
        })?;

    let mut policy =
        MeasuredBootPolicy::new(include_secureboot);
...
fn extract_scrtm_bios(
    ...
    let pcr0_events =
        handler.get_events_for_pcr_index(0);

    let mut scrtm: HashMap<String, String> =
        HashMap::new();
    let mut platform_firmware: Vec<
...
fn extract_secureboot_events(
    ...
    let pcr7_events =
        handler.get_events_for_pcr_index(7);
    ...
    for event in &pcr7_events {
                ...
                == "EV_EFI_VARIABLE_BOOT"
        {
            ...
            let event_data_hex =
                hex::encode(&event.event_data);

            // Try to detect variable name from event data
            let var_name = detect_efi_variable_name(
                &event.event_data,
            );
            ...
            match var_name.as_deref() {
                ...
                _ => {
                    ...
                    log::debug!(
                        ...
                        &event_data_hex[..std::cmp::min(32, event_data_hex.len())]
                    );
                }
            }
        }
...
fn detect_efi_variable_name(
    ...
    let name_len_bytes: [u8; 8] =
        event_data[16..24].try_into().ok()?;
    let name_len =
        u64::from_le_bytes(name_len_bytes) as usize;

    if name_len == 0
        || event_data.len() < 32 + name_len * 2
    {
        return None;
    }
...
pub fn get_eventlog_stats(
    ...
    Ok(MeasuredBootStats {
        ...
        scrtm_entries: handler
            .get_events_for_pcr_index(0)
            .len(),
        secureboot_entries: handler
            .get_events_for_pcr_index(7)
            .len(),
        algorithms: handler
...
mod tests {
    ...
    fn test_detect_efi_variable_name_pk() {
        ...
        data.extend_from_slice(&[
            0x61, 0xdf, 0xe4, 0x8b, 0xca, 0x93,
            0xd2, 0x11, 0xaa, 0x0d, 0x00, 0xe0,
            0x98, 0x03, 0x2b, 0x8c,
        ]);
    ...
    fn test_detect_efi_variable_name_too_short() {
        ...
        assert!(
            detect_efi_variable_name(&data).is_none()
        );
    }
    ...
    fn test_nonexistent_eventlog() {
        let result = generate_from_eventlog(
            Path::new("/nonexistent/event.log"),
            true,
        );
