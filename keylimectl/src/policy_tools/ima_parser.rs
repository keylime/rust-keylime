...
pub struct ParsedImaData {
    /// File path -> list of digest strings (e.g., `"sha256:abcdef..."`)
        ...
        match template_name {
            "ima" => {
                    ...
                    parse_ima_template(template_data)
                {
                    ...
                    add_digest(&mut digests, file_path.to_string(), digest_hex.to_string());
                }
            }
            "ima-ng" | "ima-sig" => {
                ...
                {
                    if detected_algorithm.is_none() {
                        if let Some(alg) = digest_str.split(':').next() {
...
pub fn parse_flat_allowlist(
    ...
    for line in content.lines() {
        ...
        if !hash.is_empty() && !file_path.is_empty() {
            add_digest(&mut digests, file_path, hash.to_string());
        }
    }
...
";
        ...
        assert_eq!(
            ...
            vec!["sha256:f1125b940480d20ad841d26d5ea253edc0704b5ec1548c891edf212cb1a9365e"]
        );
    }
...
";
    ...
    fn test_merge_digest_maps() {
        ...
        assert_eq!(base["/usr/bin/bash"][0], "sha256:aaa");
        assert_eq!(base["/usr/bin/bash"][1], "sha256:bbb");
        assert_eq!(base["/usr/bin/ls"], vec!["sha256:ccc"]);
    }
