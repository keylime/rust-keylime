...
pub struct ParsedImaData {
    ...
    /// Detected hash algorithm name (from the first valid entry's file digest)
    pub detected_algorithm: Option<String>,

    /// Detected IMA log template hash algorithm (from the template hash field length).
    ///
    /// This is the algorithm used by IMA to hash template data (the 2nd field
    /// in each IMA log line). It may differ from `detected_algorithm` which is
    /// the algorithm used for individual file content digests.
    pub detected_log_hash_alg: Option<String>,
}
...
pub fn parse_ima_measurement_list(
    ...
    for (line_num, line) in content.lines().enumerate() {
        ...
        // Detect the template hash algorithm from the hash length
        // (this is different from the file digest algorithm)
        if detected_log_hash_alg.is_none() {
            detected_log_hash_alg = detect_algorithm_from_hex(tokens[1]);
        }
...
mod tests {
    ...
    fn test_parse_ima_measurement_list() {
        // Template hashes are sha1 (40 hex chars), file digests are sha256
        let content = "\
...
";
        ...
        // Template hash algorithm (sha1) differs from file digest algorithm (sha256)
        assert_eq!(result.detected_log_hash_alg, Some("sha1".to_string()));
        assert_eq!(result.detected_algorithm, Some("sha1".to_string()));
    }
...
";
        ...
        // "aaaa" is only 4 hex chars, so log_hash_alg is not detected
        assert_eq!(result.detected_log_hash_alg, None);
    }

    #[test]
    fn test_detected_log_hash_alg_differs_from_digest_alg() {
        // Template hash is sha1 (40 hex chars), file digest is sha256
        let content = "\
10 d7026dc672344d3ee372217bdbc7395947788671 ima-ng sha256:f1125b940480d20ad841d26d5ea253edc0704b5ec1548c891edf212cb1a9365e /usr/bin/foo
";
        let f = write_temp_file(content);
        let result =
            parse_ima_measurement_list(f.path(), false, false, &[]).unwrap(); //#[allow_ci]

        // File digest algorithm is sha256
        assert_eq!(result.detected_algorithm, Some("sha256".to_string()));
        // Template hash algorithm is sha1 (detected from 40-char hex hash)
        assert_eq!(result.detected_log_hash_alg, Some("sha1".to_string()));
    }
}
