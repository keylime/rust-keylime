...
#[derive(Subcommand)]
enum PolicyAction {
    /// Upload a new runtime policy to the verifier
    Create {
        /// Policy name
        #[arg(value_name = "NAME")]
        name: String,
...
enum GenerateSubcommand {
    ...
    Runtime {
        /// IMA measurement list path (default: /sys/kernel/security/ima/ascii_runtime_measurements)
    ...
    Tpm {
        ...
        #[arg(long, value_name = "INDICES", default_value = "0,1,2,3,4,5,6,7")]
        pcrs: String,
    ...
    match cli.command {
        ...
        Some(
            ref command @ Commands::Policy {
                action: ref action @ PolicyAction::Generate { .. }
...
fn handle_no_command(config: &Config) {
    ...
    if !config.has_config_file() {
        ...
        if std::io::stdin().is_terminal() {
            eprintln!(
                "  Tip: Run `keylimectl configure` to create one."
            );
        }
        eprintln!();
    }
