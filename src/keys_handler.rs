...
pub async fn u_key(
    ...
    let mut global_current_keyset = quote_data.ukeys.lock().unwrap();
    let mut global_other_keyset = quote_data.vkeys.lock().unwrap();
    let mut global_symm_key = quote_data.payload_symm_key.lock().unwrap();
    let mut global_encr_payload = quote_data.encr_payload.lock().unwrap();
    let mut global_auth_tag = quote_data.auth_tag.lock().unwrap();
    ...
    let decrypted_key: SymmKey = decrypted_key.as_slice().try_into().unwrap();
...
pub async fn v_key(
    ...
    // must unwrap when using lock
    // https://github.com/rust-lang-nursery/failure/issues/192
    let mut global_current_keyset = quote_data.vkeys.lock().unwrap();
    let mut global_other_keyset = quote_data.ukeys.lock().unwrap();
    let mut global_symm_key = quote_data.payload_symm_key.lock().unwrap();
    let mut global_encr_payload = quote_data.encr_payload.lock().unwrap();
    let mut global_auth_tag = quote_data.auth_tag.lock().unwrap();
...
mod tests {
    ...
    async fn test_u_or_v_key(key_len: usize, payload: Option<&[u8]>) {
        ...
        let u: SymmKey = U[..key_len][..].try_into().unwrap();
        let v: SymmKey = V[..key_len][..].try_into().unwrap();
        let k = u.xor(&v).unwrap();

        let payload = payload.map(|payload| {
            let iv = b"ABCDEFGHIJKLMNOP";
            encrypt_aead(k.bytes(), &iv[..], payload).unwrap()
        });

        let encrypted_key =
            rsa_oaep_encrypt(&quotedata.pub_key, u.bytes()).unwrap();
        ...
        let encrypted_key =
            rsa_oaep_encrypt(&quotedata.pub_key, v.bytes()).unwrap();
        ...
        {
            let key = quotedata.payload_symm_key.lock().unwrap();
            assert!(key.is_some());
            assert_eq!(key.as_ref().unwrap().bytes(), k.bytes());
        }
    ...
    async fn test_verify() {
        let mut quotedata = web::Data::new(QuoteData::fixture().unwrap());

        let test_key: Vec<u8> = (0..32).collect();

        let mut symkey = quotedata.payload_symm_key.lock().unwrap();
        *symkey = Some(test_key.as_slice().try_into().unwrap());
        ...
        let result: JsonWrapper<KeylimeHMAC> =
            test::read_body_json(resp).await;
        let response_hmac = hex::decode(&result.results.hmac).unwrap();
        ...
        let expected = hex::decode("6d815226048e336305a3cf87dd5a205ae637ba1ece0716e29464ee887a04f0d784c8ace39c559dbfd65bccdd6fcb227a").unwrap();
