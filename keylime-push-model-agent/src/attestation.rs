...
mod tests {
    ...
    async fn test_send_negotiation_http_error() {
        // Lock mutex to avoid conflict accessing TPM
        let _mutex = testing::lock_tests().await;
        let tmpdir = tempfile::tempdir().expect("failed to create tmpdir");
        let config = get_testing_config(tmpdir.path());

        let negotiation_config =
            create_test_config("http://1.2.3.4", "", "", "");

        let _ = context_info_handler::init_context_info(&config, true);

        let result = send_negotiation(negotiation_config).await;
