...
impl UefiLogHandler {
    ...
    /// Replays the event log for a given PCR index and hash algorithm to calculate the final value.
    /// This is useful for verifying the log against a PCR value read from the TPM.
    pub fn replay_pcr_value(
        &self,
        pcr_index: u32,
        hash_alg_str: &str,
    ) -> Result<Option<Vec<u8>>> {
        let events_for_pcr = self.get_events_for_pcr_index(pcr_index);
        if events_for_pcr.is_empty() {
            return Ok(None);
        }

        let (mut pcr_value, hash_size) = match hash_alg_str {
            "sha1" => (vec![0u8; 20], 20),
            "sha256" => (vec![0u8; 32], 32),
            "sha384" => (vec![0u8; 48], 48),
            "sha512" => (vec![0u8; 64], 64),
            _ => {
                return Err(KeylimeError::UEFILog(format!(
                    "Unsupported hash algorithm for replay: {}",
                    hash_alg_str
                )))
            }
        };

        if (17..=22).contains(&pcr_index) {
            pcr_value = vec![0xffu8; hash_size];
        }

        // Iterate through events and "extend" the PCR value
        for event in events_for_pcr {
            // Some events, like EV_NO_ACTION, do not extend the PCR and are skipped in replay.
            if event.event_type == "EV_NO_ACTION" {
                continue;
            }

            if let Some(digest) = event.digests.get(hash_alg_str) {
                // The extend operation is HASH(old_pcr_value || event_digest)
                let mut data_to_hash = pcr_value.clone();
                data_to_hash.extend_from_slice(digest);

                pcr_value = match hash_alg_str {
                    "sha1" => Sha1::digest(&data_to_hash).to_vec(),
                    "sha256" => Sha256::digest(&data_to_hash).to_vec(),
                    "sha384" => sha2::Sha384::digest(&data_to_hash).to_vec(),
                    "sha512" => sha2::Sha512::digest(&data_to_hash).to_vec(),
                    _ => unreachable!(),
                };
            }
        }
