...
pub enum RegistrarClientBuilderError {
    ...
    /// TLS client creation error
    #[error("TLS client creation error: {0}")]
    TlsClient(#[from] anyhow::Error),
}
...
impl RegistrarClientBuilder {
    ...
    /// Set the CA certificate file path for TLS server verification
    ///
    /// When set, the registrar client will use HTTPS and verify the server's
    /// certificate against this CA certificate.
    ///
    /// # Arguments:
    ///
    ...
    pub async fn build(
        ...
        // Create the client (HTTPS with server verification or plain HTTP)
        let client = if use_tls {
            let args = https_client::TlsClientArgs {
                ca_certificate: self
                    .ca_certificate
                    .clone()
                    .unwrap_or_default(),
            ...
            https_client::get_tls_client(&args)?
        } else {
            reqwest::Client::new()
        };
...
mod tests {
    ...
    #[actix_rt::test]
    async fn test_builder_ca_certificate() {
    ...
    // Mockoon-based integration tests for registrar HTTP
    #[actix_rt::test]
