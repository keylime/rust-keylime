...
pub enum RegistrarClientBuilderError {
    ...
    /// API version is not enabled
    #[error("API version '{0}' is not enabled")]
    APIVersionNotEnabled(String),
...
impl<'a> RegistrarClientBuilder<'a> {
    ...
    pub async fn get_registrar_api_version(
        ...
        let resp: Response<KeylimeVersion> = resp.json().await?;

        let registrar_api_version = &resp.results.supported_version;

        if let Some(enabled_apis) = &self.enabled_api_versions {
            if enabled_apis.contains(&registrar_api_version.as_str()) {
                Ok(registrar_api_version.to_string())
            } else {
                warn!("Registrar at '{addr}' is using API version '{registrar_api_version}' which is not enabled: enabled versions '[{}]'", enabled_apis.join(", "));
                Err(RegistrarClientBuilderError::APIVersionNotEnabled(
                    registrar_api_version.to_string(),
                ))
            }
        } else {
            Err(RegistrarClientBuilderError::EnabledAPIVersionsNotSet)
        }
    }
    ...
    pub async fn build(
        ...
        // Get the registrar API version. In case of failure, set as "unknown"
        let registrar_api_version = self
            .get_registrar_api_version()
            .await
            .unwrap_or(UNKNOWN_API_VERSION.to_string());
...
mod tests {
    ...
    async fn test_register_agent_ok() {
        ...
        let api_response: Response<KeylimeVersion> = Response {
            code: 200.into(),
            status: "OK".to_string(),
            results: KeylimeVersion {
                supported_version: "1.2".to_string(),
            },
        };
    ...
    async fn test_register_agent_different_api() {
        ...
        let api_response: Response<KeylimeVersion> = Response {
            ...
            status: "OK".to_string(),
            results: KeylimeVersion {
                supported_version: "3.4".to_string(),
            },
        };
