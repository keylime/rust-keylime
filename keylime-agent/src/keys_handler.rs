...
async fn keys_default(req: HttpRequest) -> impl Responder {
    let (error, message, response) = match req.head().method {
        http::Method::GET => {
            let error = 400;
            let message = "URI not supported, only /pubkey and /verify are supported for GET in /keys interface";
        ...
        http::Method::POST => {
            let error = 400;
            let message = "URI not supported, only /ukey and /vkey are supported for POST in /keys interface";
            let response = HttpResponse::BadRequest()
                .json(JsonWrapper::error(error, message));
            (error, message, response)
        }
        _ => {
            let error = 405;
            let message = "Method is not supported in /keys interface";
